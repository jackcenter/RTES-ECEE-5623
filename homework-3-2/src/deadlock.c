...
void *grabRsrcs(void *threadp)
{
  ...
  const int min_delay_us = 10000;
  const int max_delay_us = 50000;
  if(threadIdx == THREAD_1)
  {
    ...
    while (pthread_mutex_timedlock(&rsrcB, &timeout) == ETIMEDOUT) {
      printf("Delay");
      pthread_mutex_unlock(&rsrcA);
      const int delay_us = (rand() % (max_delay_us + 1 - min_delay_us) + min_delay_us);
      usleep(delay_us);
      pthread_mutex_lock(&rsrcA);
      continue;
    }
