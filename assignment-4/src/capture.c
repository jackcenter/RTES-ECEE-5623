...
// Transformation
        ...
        const size_t rgb_size = (6 * size) / 4;     // Number of data points in the RGB conversion
        ...
        // const int offset = 2;                       // Distance to offset for grayscale conversion (0 = R, 1 = G, 2 = B)
        // for (size_t j = 0; j < rgb_size; ++j)
        // {
        ...
        transformation_times_ms[framecnt] = getDurationInMilliseconds(transformation_start_time_ms, transformation_stop_time_ms);
// Transformation Stop
...
// Write-back
    ...
    write_back_times_ms[framecnt] = getDurationInMilliseconds(write_back_start_time_ms, write_back_stop_time_ms);
// Write-back Stop
}
...
static int read_frame(void)
{
    ...
    switch (io)
    {
        ...
        case IO_METHOD_MMAP:
            ...
            acquisition_times_ms[framecnt] = getDurationInMilliseconds(acquisition_start_time_ms, acquisition_stop_time_ms);
            // Acquisition Stop
            ...
            frame_times_ms[framecnt] = getDurationInMilliseconds(acquisition_start_time_ms, process_end_time_ms);
...
static void mainloop(void)
{
    ...
    {
        printf("Count: %d\r", count + 1);   // Display frame number, not the index number
        for (;;)
        {
            fd_set fds;
            struct timeval tv;
